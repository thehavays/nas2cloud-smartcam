...
while true; do
  ...
  rclone copy /mnt/data "gdrive:${REMOTE_PATH}" --exclude ".deleted/**" --config /app/rclone/rclone.conf --log-level INFO
  ...
  rclone delete "gdrive:${REMOTE_PATH}" --config /app/rclone/rclone.conf --min-age ${REMOTE_RETENTION_DAYS}d --log-level INFO
  rclone rmdirs "gdrive:${REMOTE_PATH}" --config /app/rclone/rclone.conf --leave-root --log-level INFO
